...
> Extracted from the [effect](https://github.com/Effect-TS/effect/blob/main/packages/effect/src/Match.ts) library.
...
`Match.option` wraps the match result in an [Option](https://gcanti.github.io/fp-ts/modules/Option.ts.html). If a match is found, it returns `Some(value)`, otherwise, it returns `None`.
...
The `Match.either` method wraps the result in an [Either](https://gcanti.github.io/fp-ts/modules/Either.ts.html), providing a structured way to distinguish between matched and unmatched cases. If a match is found, it returns `Right(value)`, otherwise, it returns `Left(no match)`.
