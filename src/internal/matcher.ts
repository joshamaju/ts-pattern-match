...
function isEither(value: any): value is Either.Either<any, any> {
  return Either.isLeft(value) || Either.isRight(value);
}
...
export const orElse =
  ...
  <I, R, A, Pr>(
    ...
    return (input: I) => {
      ...
      return a._tag === "Right" ? a.right : f(a.left as any);
    };
  };
...
export const option: <I, F, R, A, Pr, Ret>(
  ...
  if (isEither(toEither)) {
    return pipe(
      toEither,
      Either.match(() => Option.none, Option.some),
    );
  }
  return (input: I): Option.Option<A> =>
    pipe(
      (toEither as any)(input),
      Either.match(() => Option.none, Option.some as any),
    );
}) as any;
